...
module.exports = {
    ...
    description: "Frequently looking for games in real time until you stop this command\ndelay: min - 3000, max - 60000, default - 10000",
    guildOnly: true,
    adminOnly: false,
    development: false,
    run: (message, args) => {
            ...
            channel
                ...
                .then(_ => {
                    lobbies.forEach(game => {
                        channel.send({ embed: game.embed }).then(embedMessage => messages.set(game.botid, embedMessage));
                    });
