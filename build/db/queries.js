...
const getLobbyList = () => __awaiter(void 0, void 0, void 0, function* () {
    const query = `SELECT * from gamelist`;
    return yield mysql_1.makeQuery(query);
});
...
const getPlayerWinrateForLobbyWatcher = (nickname) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    where category = "fbtmmd" 
        ...
        and duration >= 900`;
    const result = yield mysql_1.makeQuery(query);
...
const getGamesCountInfo = (nickname, minIngameTime = 900) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return (result &&
        result.map((game) => {
            return Object.assign(Object.assign({}, game), { gamesID: game.gamesId.split(","), teams: game.teams.split(","), map: lobbyParser_1.parseMapName(game.map) });
        }));
});
...
const getPlayersByGameId = (gamesID) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const players = yield mysql_1.makeQuery(query);
    return players;
});
...
const getGamesDataByIds = (gamesID, guildID) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const players = yield exports.getPlayersByGameId(gamesID);
        const query = `
    SELECT 
        id, 
        map, 
    ...
    FROM games where id in (${gamesID.join(",")})`;
        const gamesData = yield mysql_1.makeQuery(query);
        ...
        const parsedGamesData = yield Promise.all(gamesData.map((game) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const teams = uniqueFromArray_1.uniqueFromArray(gamePlayers.map((player) => player.team));
            const parsedTeams = [];
            const mapConfig = yield mapConfig_1.searchMapConfigByMapName(game.map, guildID);
            if (mapConfig)
            ...
            return {
                id: game.id,
                map: lobbyParser_1.parseMapName(game.map),
                datetime: new Date(game.datetime),
                gamename: game.gamename.replace(/#\d+/g, "").trim(),
                duration: game.duration,
                winnerTeam: yield exports.getWinnerTeamNumberFromId(game.id),
