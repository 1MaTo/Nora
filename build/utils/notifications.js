...
const pingUsersOnStart = (game, guildID) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const users = (yield nicknameToDiscordUser_1.getDiscordUsersFromNicknames(nicknames, guildID)).filter((user) => user.settings.ping_on_start);
    users.map((user) => user.user.send({ embed: response_1.info("Game started") }));
});
